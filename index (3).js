...
// ── 1. Web search ─────────────────────────────────────────────────
// GET /search?q=...&region=us&lang=en&daterange=qdr:w&offset=0
app.get("/search", async (req, res) => {
  const { q, region, lang, daterange, site, offset } = req.query;
  if (!q) return res.status(400).json({ error: "q is required" });

  const payload = {
    q: site ? `site:${site} ${q}` : q,
    gl: region || "us",
    hl: lang || "en",
    num: 10,
    start: parseOffset(req),
  };
  ...
  try {
    ...
    res.json({
      ...
      next_offset: parseOffset(req) + results.length,
      results,
    });
  } catch (err) {
...
// ── 2. Image search ───────────────────────────────────────────────
// GET /images?q=...&region=us
app.get("/images", async (req, res) => {
  const { q, region, lang, offset } = req.query;
  if (!q) return res.status(400).json({ error: "q is required" });
  ...
  try {
    ...
    const results = (data.videos || []).map((r, i) => ({
      ...
      duration_seconds: r.duration
        ? r.duration.split(":").reduce((acc, t) => 60 * acc + +t, 0)
        : null,
      platform: r.link
        ? new URL(r.link).hostname.replace("www.", "").split(".")[0]
        : null,
      published_date: r.date || null,
    }));
...
// ── 4. News search ────────────────────────────────────────────────
// GET /news?q=...&lang=en&daterange=qdr:d
...
// GET /site-search?q=...&domain=example.com
app.get("/site-search", async (req, res) => {
  const { q, domain, region, lang, offset } = req.query;
  if (!q) return res.status(400).json({ error: "q is required" });
...
app.listen(PORT, () => console.log(`x402-search backend running on :${PORT}`));
